...
contract StablePayBase is Base, IStablePay {
    ...
    string internal constant STABLE_PAY_STORAGE_NAME = "StablePayStorage";
    ...
    function allowanceHigherOrEquals(
        ...
        uint256 allowanceResult = ERC20(token).allowance(owner, spender);
        require(
    ...
    function transferFrom(
        ...
        if (amount > 0) {
            bool transferFromResult = ERC20(token).transferFrom(
                from,
                to,
                amount
            );
    ...
    function transferDiffSourceTokensIfApplicable(
        ...
        require(
            ...
            "StablePayBase Token: Final balance >= initial balance."
        );
    ...
    function calculateDiffBalance(
        ...
        require(
            ...
            "SwappingProvider Ether: Initial balance >= final balance."
        );
        uint256 used = initialBalance.sub(finalBalance);
        require(
            sentAmount >= used,
            "SwappingProvider Ether: Sent amount >= used."
        );
     ...
     */
    function calculateAndTransferToAmount(
        ...
        bool transferToPostActionResult = ERC20(order.targetToken).transfer(
            postActionAddress,
            currentToAmount
        );
        ...
        returns (bool)
    {
        ...
        if (_isTransferTokens) {
            ...
            uint256 allowanceResult = ERC20(order.sourceToken).allowance(
                msg.sender,
                address(this)
            );
            require(
            ...
            bool transferFromResult = ERC20(order.sourceToken).transferFrom(
                msg.sender,
                order.toAddress,
                order.targetAmount
            );
