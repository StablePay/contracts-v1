/**
    ...
    truffle exec ./scripts/unpauseByOwnerSwappingProvider.js --network infuraRopsten
 */
...
module.exports = async (callback) => {
    try {
        ...
        assert(swappingProviderBefore.pausedByOwner === true, 'Swapping provider must be paused by owner.');
        assert(swappingProviderBefore.pausedByAdmin === false, 'Swapping provider must not be paused by admin.');

        /******************************************************************
                                Function Invocation
        ******************************************************************/
        const unpauseSwappingProviderResult = await providerStrategy.unpauseSwappingProvider(providerKey, { from: sender });
        ...
        assert(swappingProviderAfter.pausedByOwner === false, 'Swapping provider must not be paused by owner.');
